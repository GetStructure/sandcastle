  ...
  this.sandcastle.sandboxReady(() => {
    ...
    client.on('data', (data) => {
      ...
      if (this.sandcastle.spawnReadOnly[`type${timeStamp}`]  === 'script') {
        client.end();
        sharedData = this.sandcastle.spawnReadOnly[`sandScript${timeStamp}`];
        client.write(`delete-${timeStamp}-sandScript`);
        this.onExit(methodName, sharedData );
      } else {
        sharedData = this.sandcastle.spawnReadOnly[`sandTask${timeStamp}`];
        //don't delete task, it is not complete until it returns on script data
        this.onTask(client, methodName, sharedData);
      }
    });
  });
};
