  ...
  this.sandcastle.sandboxReady(() => {
    ...
    const client = net.connect(this.sandcastle.getSocket(), () => {
      ...
      if (((Buffer.byteLength(pack) + 400)*3) > this.sandcastle.sharedObject.get_free_memory()) {
        this.onExit(methodName, EJSON.stringify({
          error: { message: 'Script with Payload exceeds your memory limit.' }
        }));
        return;
      }
      pack = Buffer.from(pack);
      const nanoSecs = process.hrtime();
      const timeStamp = ((nanoSecs[0]*1e9) + nanoSecs[1]).toString(36);
      this.sandcastle.sharedObject[`type${timeStamp}`] = 'script';
      this.sandcastle.sharedObject[`sandScript${timeStamp}`] = pack;
    ...
    client.on('data', (data) => {
      ...
      this.sandcastle.spawnReadOnly = new MMAP.Open(`/tmp/sandbox-share-${this.sandcastle.sharedFileID}`);
      if (this.sandcastle.spawnReadOnly[`type${timeStamp}`]  === 'script') {
        client.end();
        const sharedData = this.sandcastle.spawnReadOnly[`sandScript${timeStamp}`];
        this.onExit(methodName, sharedData );
      } else {
        this.onTask(client, methodName, this.sandcastle.spawnReadOnly[`sandTask${timeStamp}`]);
      }
    });
  });
};
...
Script.prototype.onExit = function(methodName, data) {
  ...
  // clean up objects from last run
  for (let key in this.sandcastle.sharedObject) {
    if (this.sandcastle.sharedObject.isData(key)) {
      delete this.sandcastle.sharedObject[key];
    }
  }
  if (this.exited) { return; }
