...
Sandbox.prototype.start = function() {
  ...
  this.server = net.createServer((c) => {
    ...
    c.on('data', (data) => {
      ...
      const readOnlySharedObject = new MMAP.Open(`/tmp/sandcastle-share-${ufid}`);
      const type = readOnlySharedObject[`type${timeStamp}`];

      // clean up objects from the last vm

      if (type  === 'script') {
        for (let key in this.mmapObject) {
          if (this.mmapObject.isData(key)) {
            delete this.mmapObject[key];
          }
        }
        this.executeScript(c, readOnlySharedObject[`sandScript${timeStamp}`] );
      } else if (type  === 'task') {
        this.answerTask(c, readOnlySharedObject[`sandTask${timeStamp}`].toString() );
      }
    });
