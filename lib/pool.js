...
Pool.prototype.createMmap = function(memoryLimit) {
  ...
  const time = ((nanoSecs[0]*1e9) + nanoSecs[1]).toString(36);
  this.ufid = pid + time;
  const tempMemFile = `/tmp/sandcastle-share-${this.ufid}`;
  // setup with a portion of memory (we only want a ~7th of the max)
  let mmapMemory = memoryLimit * 150;
...
Pool.prototype.kill = function() {
  ...
  try { fs.unlinkSync(`/tmp/sandcastle-share-${this.ufid}`); } catch (e) {}
};
