...
SandCastle.prototype.createMmap = function() {
  const tempMemFile = `/tmp/sandcastle-share-${this.sharedFileID}`;
  // setup with a portion of memory (we only want a ~7th of the max)
  let mmapMemory = this.memoryLimitMB * 150;
  if (mmapMemory === 0) { mmapMemory = 5120; } //KB
  this.sharedObject = new MMAP.Create(tempMemFile, mmapMemory, 10);
};
...
SandCastle.prototype.spawnSandbox = function() {
  ...
  this.sandbox = spawn(this.spawnExecPath, [
    ...
    'sandbox',
    `--socket=${this.socket}`,
    `--memoryLimit=${this.memoryLimitMB.toString()}`,
    `--mmapPath=/tmp/sandbox-share-${this.sharedFileID}`
  ], { cwd: this.cwd, env: { } });
...
SandCastle.prototype.kill = function() {
  ...
  this.sandbox.kill('SIGINT');

  // take out shared file
  if (this.singleMode) {
    try { fs.unlinkSync(`/tmp/sandcastle-share-${this.sharedFileID}`); } catch (e) {}
  } else {
    // extra cleanup, timout for process to possibly finish cleanup (unlink will usually fail for sandbox share)
    setTimeout(() => {
      try { fs.unlinkSync(`/tmp/sandbox-share-${this.sharedFileID}`); } catch (e) {}
      try { fs.unlinkSync(`/tmp/sandcastle-share-${this.sharedFileID}`); } catch (e) {}
      this.sandbox.kill(SIGHUP);
    }, 1);
  }
